        ...
        <div className={`mb-4 rounded-xl border p-3 text-xs ${liveStatus.status.live ? 'border-emerald-500/70 bg-emerald-500/10 text-emerald-100' : 'border-amber-500/70 bg-amber-500/10 text-amber-100'}`}>
          <div className="flex items-center justify-between gap-2">
            <p>
              ...
              {' '}Providers: newsapi={String(liveStatus.status.providers.newsapi)}, gnews={String(liveStatus.status.providers.gnews)}, rss2json={String(liveStatus.status.providers.rss2json)}. Last source: {liveStatus.status.lastProvider.toUpperCase()} ({liveStatus.status.lastReason}).
            </p>
