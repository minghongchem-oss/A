  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
      <article className="chart-card">
        {factCheck.error?.includes('API key missing') && (
          <div className="mb-3 rounded-lg border border-amber-500/70 bg-amber-500/15 p-2 text-xs text-amber-100">{factCheck.error}</div>
        )}
        ...
        <form className="mt-3 space-y-2" onSubmit={(event) => { event.preventDefault(); setSubmitted(input); }}>
        ...
        <div className="mt-4 rounded-xl border border-slate-700 bg-slate-950/60 p-3">
          ...
          {isUrl ? (
            <iframe title="article preview" src={input} className="h-72 w-full rounded border border-slate-700" />
          ) : (
        ...
        <p className="method-note">Method: regex + mock claim spans + linked fact-check sources. Users must verify via source links.</p>
      </article>
      ...
      <aside className="space-y-4">
          ...
          <ul className="mt-3 space-y-2 text-xs text-slate-200">
            {sampleAnnotations.map((item) => (
              <li key={item.id} className="rounded-lg border border-slate-700 bg-slate-950/60 p-2">
                ...
                <div className="mt-2 space-y-1 text-slate-300">{item.factChecks.map((fc) => <p key={fc}>• {fc}</p>)}</div>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="chart-card">
          ...
          <p className="method-note">Sources: GNews index + checker references (PolitiFact / Snopes / FactCheck.org).</p>
        </article>
      </aside>
    </section>
  );
