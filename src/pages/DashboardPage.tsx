  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4 text-sm text-cyan-100">
        ...
        <button onClick={() => void volume.refresh()} className="rounded-lg bg-cyan-500 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-cyan-400">
          Refresh data
        </button>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <article className="chart-card">
          ...
          <p className="method-note">Sources: NewsAPI + AllSides-style cohort mapping. Data date: {volume.fetchedAt.slice(0, 10)}.</p>
        ...
        <article className="chart-card">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <div className="h-60"><ResponsiveContainer><RadarChart data={omissionRadar}><PolarGrid /><PolarAngleAxis dataKey="topic" tick={{ fontSize: 10 }} /><Radar name="Left" dataKey="Left" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.3} /><Radar name="Right" dataKey="Right" stroke="#f97316" fill="#f97316" fillOpacity={0.25} /><Legend /></RadarChart></ResponsiveContainer></div>
            <div className="h-60"><ResponsiveContainer><PieChart><Pie data={pieData} dataKey="value" nameKey="name" outerRadius={85} fill="#14b8a6" label /><Tooltip /></PieChart></ResponsiveContainer></div>
          </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4" onClick={() => setSelectedSide(null)}>
          <div className="max-h-[80vh] w-full max-w-2xl overflow-auto rounded-xl border border-slate-700 bg-slate-950 p-4" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-sm font-semibold">{selectedSide} source articles</h3>
              <button className="rounded border border-slate-600 px-2 py-1 text-xs" onClick={() => setSelectedSide(null)}>Close</button>
            </div>
            {(sideArticles.length ? sideArticles : [] as NewsArticle[]).map((article) => (
              <a key={article.url} href={article.url} target="_blank" rel="noreferrer" className="mb-2 block rounded border border-slate-700 p-2 text-sm hover:border-cyan-500">
                <p>{article.title}</p><p className="text-xs text-slate-400">{article.source} · {article.date}</p>
              </a>
            ))}
